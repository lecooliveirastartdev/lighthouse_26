...
def find_csv_files(directory: Path) -> list[Path]:
    ...
    # Verifica se o caminho realmente representa um diretório.
    if not directory.is_dir():
        raise NotADirectoryError(
            f"O caminho informado não é um diretório: {directory}"
        )

    # Localiza apenas arquivos terminados em .csv.
    csv_files = sorted(
        directory.glob("*.csv")
    )

    # Interrompe a execução caso nenhum CSV seja encontrado.
    if not csv_files:
        raise FileNotFoundError(
...
def connect_database(
    ...
    As informações da conexão são recebidas em tempo de execução,
    evitando dependência de credenciais específicas da máquina
    onde o script foi desenvolvido.
    """
...
def prepare_value(value: str):
    """
    Prepara um valor vindo do CSV para inserção no PostgreSQL.

    Importante:
    não é realizada limpeza ou correção do conteúdo.

    Uma string vazia é convertida para None para que o PostgreSQL
    registre a ausência de valor como NULL.

    Os demais valores são preservados exatamente como foram
    encontrados no arquivo CSV.
    """
...
def load_csv(
    ...
    # O nome do arquivo sem a extensão .csv corresponde
    # ao nome da tabela criada anteriormente no PostgreSQL.
    ...
    with file_path.open(
        ...
        # O CSV precisa possuir cabeçalho para que seja possível
        # relacionar os valores às colunas da tabela.
        ...
        # Monta dinamicamente a instrução:
        #
        # INSERT INTO tabela (coluna1, coluna2, ...)
        # VALUES (%s, %s, ...)
        #
        # psycopg.sql.Identifier protege nomes de tabelas
        # e colunas durante a construção da consulta.
        ...
        with connection.cursor() as cursor:
            ...
            # Percorre cada registro do CSV sem realizar
            # limpeza ou alteração do conteúdo.
            for row in reader:
...
def load_all_csvs(
    ...
    """
    Carrega todos os arquivos CSV encontrados no diretório.

    Returns
    -------
    dict[str, int]
        Dicionário com o nome da tabela e a quantidade de
        registros inseridos.
    """
    ...
    # Abre a conexão utilizando os parâmetros recebidos
    ...
    with connect_database(
        ...
        # Confirma as inserções somente após o processamento
